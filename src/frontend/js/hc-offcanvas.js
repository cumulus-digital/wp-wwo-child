	...
	$( function () {
		...
		if ( $originalNav && $originalNav.length ) {
			...
			const Nav = new hcOffcanvasNav( $originalNav.get( 0 ), {
				...
				navClass: $originalNav.hasClass( 'has-social-menu' )
					? 'has-social-menu'
					: '',
			...
			if ( Nav && Nav.hasOwnProperty( 'on' ) ) {
				...
				Nav.on( 'open', ( e ) => {
					...
					let $firstTab = $Nav.find(
						'li.level-open:last .nav-content:first [tabindex=0]:first'
					);
					if ( ! $firstTab.length ) {
						$firstTab = $Nav.find(
							'.nav-content:first [tabindex=0]:first'
						);
					}
